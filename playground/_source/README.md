...
- `icon.png`: the K2 icon in the tab, the one Karl2D's `make_karl2d_icon` draws and a web build puts in its own tab. Karl2D generates it in code rather than shipping a file, so this is a copy of what it produced (64x64, the size a web build uses); if that drawing ever changes, take the favicon a running example sets and save it again.
